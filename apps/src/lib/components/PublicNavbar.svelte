<script lang="ts">
  ...
  const useTransparent = $derived(isHome && !scrolled);
  ...
  const links = [
    ...
    { name: 'Paket Catering', href: '/paket-catering' },
...
<nav 
    class="fixed top-0 left-0 right-0 z-[100] transition-all duration-500 border-b
           {!useTransparent 
             ? 'bg-white/90 backdrop-blur-xl border-zinc-100 py-4 shadow-sm' 
...
>
  <div class="container mx-auto px-6 flex items-center justify-between">
    ...
    <a href="/" class="flex items-center gap-3 group">
      ...
      <span class="text-xl font-black tracking-tighter {!useTransparent ? 'text-brand-charcoal' : 'text-white'} transition-colors">
      ...
      {#each links as link}
        <a 
          ...
          class="text-xs font-black uppercase tracking-widest transition-all hover:text-brand-primary
                   {!useTransparent ? 'text-zinc-500' : 'text-white/80 hover:text-white'}"
        >
          {link.name}
        </a>
      {/each}
    ...
    <div class="flex items-center gap-4">
      <button 
        ...
        class="relative p-2 rounded-xl transition-all {!useTransparent ? 'text-brand-charcoal hover:bg-zinc-100' : 'text-white hover:bg-white/10'}"
      >
        <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
        </svg>
        {#if cart.totalItems > 0}
          <span class="absolute top-0 right-0 w-5 h-5 bg-brand-primary text-white text-[10px] font-black rounded-full flex items-center justify-center border-2 {!useTransparent ? 'border-white' : 'border-brand-charcoal'}">
            {cart.totalItems}
          </span>
        {/if}
      </button>
      ...
      <button aria-label="Menu Mobile" class="md:hidden p-2 {!useTransparent ? 'text-brand-charcoal' : 'text-white'}">
        <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
