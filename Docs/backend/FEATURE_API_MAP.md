...
| **F08** | Package Request | Found | `/api/package-requests`, `/api/package-requests/[id]/status`, `/api/package-requests/[id]/convert`, `apps/src/lib/server/services/packageRequestService.ts`, `apps/src/lib/server/repositories/packageRequestRepository.ts` | `features/F08_package_request.md` | Endpoint submit/get request katering, PATCH status review admin, POST convert ke order aktif, service logic, dan database repository operations |
| **F09** | Customer Dashboard Home | Found | `/api/orders`, `/api/package-requests` | `features/F09_customer_dashboard_home.md` | Agregasi data client-side untuk perhitungan statistik reaktif (jumlah pesanan aktif, pesanan selesai, sisa pembayaran, status request paket) |
